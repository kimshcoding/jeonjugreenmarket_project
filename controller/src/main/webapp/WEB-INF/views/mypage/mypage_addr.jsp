...
/*배송지 수정모달창에서 저장버튼 클릭시 입력한 정보들을 updatedInfo에 담아서 컨트롤러로 보냄.*/
...
/*새로운 배송지를 저장할 ajax 통신 */
$(document).on('click', '#saveButton', function() {
    var addAddrInfo = {
    	addrPost: $('#modal_sample4_postcode').val(),
        addr: $('#modal_sample4_roadAddress').val(),
        addrDetail: $('#modal_sample4_detailAddress').val(),
        phone: $('#addr-phone').val(),
        name: $('#addr-name').val(),
        def: $('#addr-def').is(':checked') ? 'Y' : 'N'
    };
    
    $.ajax({
        url: 'add_address.do',
        method: 'POST',
        data: addAddrInfo, // 객체를 직접 전달하면 jQuery가 쿼리 스트링으로 변환
        success: function(data) {
        	console.log(data);
        	if(data==0){
        		alert("배송지가 등록되지 않았습니다.");
        	}else{
    ...
    function sample4_execDaumPostcode() {
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                // 법정동명, 건물명 추가 등의 기존 로직 유지
                var extraRoadAddr = ''; // 참고 항목 변수 삭제됨
                ...
                // 우편번호, 도로명 주소 정보를 모달의 해당 필드에 설정
                document.getElementById('modal_sample4_postcode').value = data.zonecode;
                document.getElementById("modal_sample4_roadAddress").value = roadAddr;
                    ...
                    <div class=" row">
						<div class="col-lg-12 text-left">
                            ...
                            <div class="blog__sidebar__item__tags" style="display: flex; justify-content: flex-end;">
                                <button onclick="sample4_execDaumPostcode()">+ 새 배송지 추가</button>
						  ...
						  <div class="modal-dialog" role="document">
						    <div class="modal-content">
						      ...
						      <div class="modal-body">
							      <div class="form-group">
							      <label>주소</label>
							        <input type="text" id="modal_sample4_postcode" placeholder="우편번호" class="form-control mb-2" name="addrPost">
							        <input type="text" id="modal_sample4_roadAddress" placeholder="도로명주소" class="form-control mb-2" name="addr">
							        <input type="text" id="modal_sample4_detailAddress" placeholder="상세주소" class="form-control mb-2" name="addrDetail">
							       </div> 
							   
							        <div class="form-group">
							          <label for="edit-name">받으실 분</label>
							          <input type="text" class="form-control" id="addr-name" name="name">
							        </div>
							  
							        <div class="form-group">
							          <label for="edit-phone">휴대폰 번호</label>
							          <input type="text" class="form-control" id="addr-phone" name="phone">
							        </div>
						    		<div class="form-group">
							           <label for="def">기본배송지로 저장</label>
							           <input type="checkBox" name="def" id="addr-def" value="N"> 예
							        </div>
						      </div>
						      <div class="modal-footer">
